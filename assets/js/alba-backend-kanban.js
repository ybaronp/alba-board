...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.alba-card').forEach(card => {
        card.addEventListener('click', function(e) {
            ...
            fetch(
                albaBoard.ajaxurl +
                '?action=alba_get_card_details_admin&card_id=' + encodeURIComponent(cardId) +
    ...
    // Helper to initialize Select2 on fields (needed for modal reload)
    function initAlbaSelect2() {
        if (typeof jQuery !== 'undefined' && jQuery().select2) {
            jQuery('.alba-select2').select2({
                width: '100%',
